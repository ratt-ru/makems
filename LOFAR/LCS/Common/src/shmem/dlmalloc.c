...
#else
#define SHMEM_ALLOC_INSERT_SEGMENT_ADDRESS(p)	\
{						\
  char* sp;					\
  sp = (char*)p + (2*sizeof(INTERNAL_SIZE_T));	\
  *(void**)sp=(void*)shm_segbasemap_find((void*)p);	\
}
...
#else
#define MIN_CHUNK_SIZE        (sizeof(struct malloc_chunk)+(2*SIZE_SZ))
#endif
...
#else
#define request2size(req)                                         \
  (((req) + (3*SIZE_SZ) + MALLOC_ALIGN_MASK < MINSIZE)  ?             \
   MINSIZE :                                                      \
   ((req) + (3*SIZE_SZ) + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK)
#endif
...
#ifdef SHMEM_ALLOC
      SHMEM_ALLOC_INSERT_SEGMENT_ADDRESS(p);
#endif
      return chunk2mem(p);
    }
  }
...
#ifdef SHMEM_ALLOC
      SHMEM_ALLOC_INSERT_SEGMENT_ADDRESS(victim);
#endif
...
#ifdef SHMEM_ALLOC
	SHMEM_ALLOC_INSERT_SEGMENT_ADDRESS(oldp);
#endif
...
#ifdef SHMEM_ALLOC
	  SHMEM_ALLOC_INSERT_SEGMENT_ADDRESS(newp);
#endif
